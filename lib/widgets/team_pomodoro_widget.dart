...
class TeamPomodoroGrid extends StatefulWidget {
  const TeamPomodoroGrid({super.key});

  @override
  State<TeamPomodoroGrid> createState() => _TeamPomodoroGridState();
}

class _TeamPomodoroGridState extends State<TeamPomodoroGrid> {
  ...
  void initState() {
    super.initState();
    _fetchClubCategories(); // 앱이 시작될 때 카테고리 데이터를 조회합니다.
  }

  Future<void> _fetchClubCategories() async {
    try {
      ...
      if (userId != null) {
        ...
        List<CategoryModel> categoryList =
            await _apiService.getClubCategories(int.parse(userId));
        // 상태를 업데이트합니다.
        setState(() {
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      itemCount: categories.length, // 카테고리 목록 수 + 추가 버튼
      itemBuilder: (BuildContext context, int index) {
        ...
        return GestureDetector(
          ...
          child: CategoryTile(
            ...
            color: const Color(0xfff2f3f5), // 가정한 프로퍼티 사용
            textColor: const Color(0xff84888F),
          ),
        );
      },
    );
